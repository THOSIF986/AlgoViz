...
export const LoginPage: React.FC = () => {
  ...
  const createUserProfile = (userData: { id?: string; name?: string; email?: string; photoURL?: string | null; provider?: 'email' | 'google' }) => {
    const userProfile = {
      id: userData.id || `user_${Date.now()}`,
      name: userData.name || formData.name || 'User',
      email: userData.email || formData.email,
      photoURL: userData.photoURL !== null ? userData.photoURL : undefined,
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        let user = existingUsers.find((u: { email: string }) => u.email === formData.email);
        ...
        if (!storedPassword) {
          ...
          setErrors({ password: 'Please sign up again to set a password' });
        ...
        const userIndex = existingUsers.findIndex((u: { email: string }) => u.email === formData.email);
        if (userIndex >= 0) {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      const existingUsers = JSON.parse(localStorage.getItem('algovis_users') || '[]');
      const userIndex = existingUsers.findIndex((u: { email: string }) => u.email === mockGoogleUser.email);
      ...
      const userProfile = createUserProfile({
        ...mockGoogleUser,
        photoURL: mockGoogleUser.photoURL !== null ? mockGoogleUser.photoURL : undefined
      });
